...
Like before lets create three variables but this time we call them `dogYearOfBirth` and `dogYearFuture`. Instead of `age`, we will be calculating the dog's age, so this variable will be `dogAge`.  We add an extra variable called `shouldShowResultInDogYears`. If it is `true` we should show the result in dog years, if it is false we should show it in human years. What do we call this type of variable? Now log this string out:
